...
def main():
    ...
    if user_input.startswith('/brandifai:task-plan '):
        # Raw text format: /brandifai:task-plan task-name
        is_brandifai_task_plan = True
        task_name_slug = user_input[21:].strip()
    elif '<command-name>/brandifai:task-plan</command-name>' in user_input:
        ...
        if args_match:
            task_name_slug = args_match.group(1).strip()

    if is_brandifai_task_plan:
